...
def callback(call):
    ...
    try:
        ...
        if data == "back_start":
            ...
            wait_times = {"mustaqil": "3-5", "yozma": "5-8", "amaliy": "5-8", "diplom": "15-20"}
            wt = wait_times.get(order["work_type"], "5-10")
            bot.send_message(
                order["user_id"],
                "Tolovingiz tasdiqlandi!\n\n"
                "Ish tayyorlanmoqda, " + wt + " daqiqa kuting...\n"
                "Buyurtma #" + str(order_id) + "\n\n"
                "Ish tayyor bulganda fayl yuboriladi, sabr qiling!"
            )
            ...
            def gen():
                try:
                    ...
                    filename = order["work_type"] + "_" + safe + ".docx"
                    ...
                    r = t.add_run(WORK_TYPES[order["work_type"]]["name"].upper())
                    r.bold = True
                    r.font.size = Pt(14)

                    s = doc.add_paragraph()
                    s.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    s.add_run("Mavzu: " + order["topic"]).bold = True
                    doc.add_paragraph()
                    ...
                    for line in work_text.split("\n"):
                        ...
                        is_heading = (
                            line.isupper() or
                            any(line.startswith(x) for x in ["I BOB", "II BOB", "III BOB",
                                "KIRISH", "XULOSA", "MUNDARIJA", "ADABIYOT", "ANNOTATSIYA",
                    ...
                    from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton
                    kb = InlineKeyboardMarkup()
                    kb.add(InlineKeyboardButton("Yangi buyurtma berish", callback_data="back_start"))
                    bot.send_message(
                        order["user_id"],
                        "Yangi ish buyurtma qilmoqchimisiz?",
                        reply_markup=kb
                    )
                except Exception as e:
